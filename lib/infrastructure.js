...
class Infrastructure extends EventEmitter {
  constructor (options) {
    ...
    this.infraYmlPath = path.join(this.dataDir, '/infrastructure.yml');
    this._representation = Immutable.fromJS({ devices: {} });
      this._devicesStorage = new DevicesStorage({ dataDir: this.dataDir });

	let watcher = chokidar.watch(this.infraYmlPath );
	this.isUpdate  = false;
    watcher.on('change', async () => {
		console.log(" YML file updated");
		this.isUpdate  = true;
		this.loadinfraYml();
		this.isUpdate  = false;
	});
      ...
      this._representation.get('devices').forEach(async (device, deviceId) => {
        ...
        if (!doc) {
          ...
          await this._devicesStorage.insertDevice({
            id: deviceId,
            location: device.get('location').toJS(),
 ...
 async start () {
	...
	// If devices don't exist in db let's create them, otherwise populate object
      ...
      dispatcher.on('homieMessage', async (parsed) => {
		...
		log.debug(`NBM_2a: Not device , Node property`, {propertyName: parsed.property.name, propertyValue: parsed.property.value});
        ...
        if (parsed.type === SET_NODE_PROPERTY) {
          ...
          log.debug(`NBM_3 device ${parsed.deviceId} node ${parsed.nodeId} - ${parsed.property.name} wanted to ${convertedValue}`);
